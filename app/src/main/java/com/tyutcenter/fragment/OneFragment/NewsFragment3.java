...
import com.tyutcenter.adapter.FragmentNewsTZGGAdapter;
...
public class NewsFragment3 extends BaseFragment<MainPresenter.MainUiCallback> implements MainPresenter.NewsUi{
    ...
    public void getNewsCallback(List<News> newsList,boolean isRefresh) {
        ...
        CommonUtil.setShardPString("news3", mGson.toJson(mAdapter.getData()));
    ...
    public static NewsFragment3 getInstance(){
        if (fragment_news1 == null){
            fragment_news1 = new NewsFragment3();
        }
    ...
    protected void lazyLoad() {
        String news1 = CommonUtil.getShardPStringByKey("news3");
        String news1_count = CommonUtil.getShardPStringByKey("news3_count");
        ...
        if (CommonUtil.isStrEmpty(news1_count)){
            String url = Constants.base_url+"/xyxw/tzgg.htm";
            getCallbacks().getNewsCountTZGG(url);
        }else {
